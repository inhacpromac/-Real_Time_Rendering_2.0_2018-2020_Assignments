package com.inhacpromac.checkerboard;
...
public class GlesView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
	...
	private void initialize() {
		...
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(rectangleTexCoord.length * 4);//(ArrayLength*DatatypeSize)
	...
	private int loadGLTexture() {
		...
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(checkImageHeight * checkImageWidth * 4);
		byteBuffer.order(ByteOrder.nativeOrder());
		byteBuffer.put(checkImage);//put takes single dimenatiosnal array
		byteBuffer.position(0);
		
		Bitmap bitmap = Bitmap.createBitmap(checkImageWidth, checkImageHeight, Bitmap.Config.ARGB_8888);//last parameter is: ImageFormat
    ...
    private void display() {
		...
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, 12 * 4, positionBuffer, GLES32.GL_DYNAMIC_DRAW);
		GLES32.glVertexAttribPointer(GLESMacros.AMC_ATTRIBUTE_POSITION, 3, GLES32.GL_FLOAT, false, 0, 0);
		GLES32.glEnableVertexAttribArray(GLESMacros.AMC_ATTRIBUTE_POSITION);
		...
		final float[] tiltedRectangleVertices = new float[]
		{
		...
		//1: Allocate the buffer directly from the native memory(Not from VM Memeory(Unmanaged memory)).
		byteBuffer = ByteBuffer.allocateDirect(tiltedRectangleVertices.length * 4);//(ArrayLength*DatatypeSize)
		//2: Arrange the byte order of buffer in native byte order.
        byteBuffer.order(ByteOrder.nativeOrder());
		//3: Create the float type buffer and Convert our ByteBuffer in FloatBuffer.
        positionBuffer = byteBuffer.asFloatBuffer();
		//4: Now, put your array into "cooked buffer".
        positionBuffer.put(tiltedRectangleVertices);
		//5: Set the array at "0TH" position of the buffer.
        positionBuffer.position(0);
	...
	private void uninitialize() {
		...
		if(texImage[0] != 0)
        {
            GLES32.glDeleteBuffers(1, texImage, 0);
            texImage[0] = 0;
        }
