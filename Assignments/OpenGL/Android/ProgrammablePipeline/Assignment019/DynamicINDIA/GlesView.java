package com.inhacpromac.dynamic_india;
...
public class GlesView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
	...
	private float transI1 = -10.0f;
	private float transN = 15.0f;
	private float transD = 15.0f;
	private float transI2 = -15.0f;
	private float transA = 15.0f;

	private boolean bDrawN = false;
	private boolean bDrawI2 = false;
	private boolean bDrawA = false;
	private boolean bDrawD = false;
	...
	private void initialize() {
		...
		//1: Allocate the buffer directly from the native memory(Not from VM Memeory(Unmanaged memory)).
		byteBuffer = ByteBuffer.allocateDirect(Color.length * 4);//(ArrayLength*DatatypeSize)
		//2: Arrange the byte order of buffer in native byte order.
        byteBuffer.order(ByteOrder.nativeOrder());
		//3: Create the float type buffer and Convert our ByteBuffer in FloatBuffer.
        positionBuffer = byteBuffer.asFloatBuffer();
		//4: Now, put your array into "cooked buffer".
        positionBuffer.put(Color);
		//5: Set the array at "0TH" position of the buffer.
        positionBuffer.position(0);

		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, Color.length * 4, positionBuffer, GLES32.GL_STATIC_DRAW);
    ...
    private void display() {
		...
		Matrix.translateM(modelViewMatrix, 0, transI1, 0.0f, -20.0f);
		...
		//Do necessary Trasformation
		Matrix.translateM(modelViewMatrix, 0, -1.0f, transN, -20.0f);
        ...
        //Draw necessary Scence
        GLES32.glDrawArrays(GLES32.GL_LINES, 2, 7);
		...
		//Do necessary Trasformation
		Matrix.translateM(modelViewMatrix, 0, 1.0f, transI2, -20.0f);
		...
		//Do necessary Trasformation
		Matrix.translateM(modelViewMatrix, 0, transA, 0.0f, -20.0f);
        ...
        //Draw necessary Scence
        GLES32.glDrawArrays(GLES32.GL_LINES, 18, 8);
		...
		//Do necessary Trasformation
		Matrix.translateM(modelViewMatrix, 0, 0.0f, 0.0f, transD);
        ...
        //Draw necessary Scence
        GLES32.glDrawArrays(GLES32.GL_LINES, 8, 8);
