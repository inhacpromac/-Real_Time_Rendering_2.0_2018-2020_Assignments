...
HRESULT initialize(void)
{
	...
	const char* pixelShaderSourceCode =
		...
		"		\n" \
		"		phong_ads_color = ambientZero + diffuseZero + specularZero + ambientOne + diffuseOne + specularOne;" \
		"   }" \
		"   else" \
		"	{" \
	...
	float pyramidVertices[] =
	{
		...
		0.0f, 1.0f, 0.0f,		//fronttop
		-1.0f, -1.0f, 1.0f,		//frontleft
		1.0f, -1.0f, 1.0f,		//frontright

		//triangle of right side
		0.0f, 1.0f, 0.0f,		//righttop
		1.0f, -1.0f, 1.0f,		//rightleft
		1.0f, -1.0f, -1.0f,		//rightright

		//triangle of back side
		0.0f, 1.0f, 0.0f,		//backtop
		1.0f, -1.0f, -1.0f,		//backright
		-1.0f, -1.0f, -1.0f,	//backleft

		//triangle of left side
		0.0f, 1.0f, 0.0f,		//lefttop
		-1.0f, -1.0f, -1.0f,	//leftleft
		-1.0f, -1.0f, 1.0f,		//leftright
	};
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Failed For Vertex Position Pyramid Buffer.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Succeeded For Vertex Position Pyramid Buffer.\n");
		fclose(gpFile);
	}
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Position_Pyramid,
		NULL,
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(pyramidNormals);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Normal_Pyramid);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Failed For Vertex Normal Pyramid Buffer.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Succeeded For Vertex Normal Pyramid Buffer.\n");
		fclose(gpFile);
	}

	//copy normals into above buffer
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Normal_Pyramid,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresource);
...
void display(void)
{
	...
	UINT offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Position_Pyramid,
		&stride, &offset);

	//select which vertex buffer normal to display
	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Normal_Pyramid,
		&stride, &offset);
	...
	if (gbLighting == true)
	{
		...
		constantBuffer.lightPositionZero = XMVectorSet(lightZeroPosition[0], lightZeroPosition[1], lightZeroPosition[2], lightZeroPosition[3]);
		constantBuffer.lightPositionOne = XMVectorSet(lightOnePosition[0], lightOnePosition[1], lightOnePosition[2], lightOnePosition[3]);
