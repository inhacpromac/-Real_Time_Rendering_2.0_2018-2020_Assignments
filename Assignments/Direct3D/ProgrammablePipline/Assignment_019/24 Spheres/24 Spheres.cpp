...
float materialAmbient[totalNumberOfColumns][totalNumberOfRows][4] = {
	{
...
float materialDiffuse[totalNumberOfColumns][totalNumberOfRows][4] = {
	{
...
float materialSpecular[totalNumberOfColumns][totalNumberOfRows][4] = {
	{
...
float materialShininess[totalNumberOfColumns][totalNumberOfRows] = {
	{
		0.6f * 128.0f,
		0.1f * 128.0f,
		0.3f * 128.0f,
...
void display(void)
{
	...
	//Set viewport
	D3D11_VIEWPORT d3dMyViewPort;
	//d3dMyViewPort.TopLeftX = 0;//this is updating on fly....
	//d3dMyViewPort.TopLeftY = 0;
	d3dMyViewPort.Width = (float)vWidth;
	d3dMyViewPort.Height = (float)vHeight;
	d3dMyViewPort.MinDepth = 0.0f;
	d3dMyViewPort.MaxDepth = 1.0f;

	for (int column = 0; column < totalNumberOfColumns; ++column)
	{
		for (int row = 0; row < totalNumberOfRows; ++row)
		{
			d3dMyViewPort.TopLeftX = vX + (column * vWidth);//IMPORTANT
			d3dMyViewPort.TopLeftY = vY + (row * vHeight);
			gpID3D11DeviceContext->RSSetViewports(1, &d3dMyViewPort);
			...
			if (gbLighting == true)
			{
				if (lightXRotate)
				{
					...
					lightPosition[1] = sinf(angle) * 100.0f - 3.0f;//only sin()for double type... sinef() for float type....
					lightPosition[2] = -cosf(angle) * 100.0f - 3.0f;
				}
				else if (lightYRotate)
				{
					lightPosition[0] = sinf(angle) * 100.0f - 3.0f;
				...
				{
					lightPosition[0] = sinf(angle) * 100.0f - 3.0f;
					lightPosition[1] = cosf(angle) * 100.0f - 3.0f;
				...
				constantBuffer.ka = XMVectorSet(materialAmbient[column][row][0], materialAmbient[column][row][1], materialAmbient[column][row][2], materialAmbient[column][row][3]);
				constantBuffer.kd = XMVectorSet(materialDiffuse[column][row][0], materialDiffuse[column][row][1], materialDiffuse[column][row][2], materialDiffuse[column][row][3]);
				constantBuffer.ks = XMVectorSet(materialSpecular[column][row][0], materialSpecular[column][row][1], materialSpecular[column][row][2], materialSpecular[column][row][3]);
				constantBuffer.materialShininess = materialShininess[column][row];
			}
			else
			{
