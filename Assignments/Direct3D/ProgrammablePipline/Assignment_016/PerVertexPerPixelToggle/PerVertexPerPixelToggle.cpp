...
ID3D11InputLayout* gpID3D11InputLayoutPerPixel = NULL;
...
float materialAmbient[] = { 0.75f, 0.75f, 0.75f, 1.0f };
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 't':
			if (gbLighting == true)
			{
				gbPerPixelLighting = !gbPerPixelLighting;
				if (gbPerPixelLighting == true)
				{
					...
					gpID3D11DeviceContext->IASetInputLayout(gpID3D11InputLayoutPerPixel);
				}
				else
				{
					...
					gpID3D11DeviceContext->IASetInputLayout(gpID3D11InputLayoutPerVertex);
				}
			}
			break;
		default:
...
HRESULT initialize(void)
{
	...
	const char* vertexShaderSourceCodePerVertex =
		...
		"       float3 transformedNormal = normalize(mul((float3x3)mul(viewMatrix, worldMatrix), (float3)normal));" \
		"       float3 lightDirection = (float3)normalize(lightPosition - eye_coordinates);" \
		"       float tn_dot_ld = max(dot(transformedNormal, lightDirection), 0.0);" \
		"		float4 ambient = la * ka;" \
		"		float4 diffuse = ld * kd * tn_dot_ld;" \
		"		float3 reflectionVector = reflect(-lightDirection, transformedNormal);" \
		"		float3 viewVector = normalize(-eye_coordinates.xyz);" \
	...
	ID3DBlob* pID3DBlob_ErrorPerVertex = NULL;
	...
	hr = D3DCompile(vertexShaderSourceCodePerVertex,
		...
		&pID3DBlob_ErrorPerVertex);

	if (FAILED(hr))
	{
		if (pID3DBlob_ErrorPerVertex != NULL)
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "D3DCompile() Failed For Vertex Shader Per Vertex : %s.\n", (char*)pID3DBlob_ErrorPerVertex->GetBufferPointer());
			fclose(gpFile);
			pID3DBlob_ErrorPerVertex->Release();
			pID3DBlob_ErrorPerVertex = NULL;
			return(hr);
		}
	}
	else
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateVertexShader() Per Vertex Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateVertexShader() Per Vertex Succeeded. \n ");
		fclose(gpFile);
	}
	...
	const char* pixelShaderSourceCodePerVertex =
		...
		"{" \
		"   float4 out_color = phong_ads_color;" \
		"   return out_color;" \
		"}";
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreatePixelShader() Per Vertex Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreatePixelShader() Per Vertex Succeeded.\n ");
		fclose(gpFile);
	}

	//create and set input layout
	D3D11_INPUT_ELEMENT_DESC inputElementDescPerVertex[2];
	inputElementDescPerVertex[0].SemanticName = "POSITION";
	inputElementDescPerVertex[0].SemanticIndex = 0;
	inputElementDescPerVertex[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	inputElementDescPerVertex[0].InputSlot = 0;
	inputElementDescPerVertex[0].AlignedByteOffset = 0;
	inputElementDescPerVertex[0].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	inputElementDescPerVertex[0].InstanceDataStepRate = 0;

	inputElementDescPerVertex[1].SemanticName = "NORMAL";
	inputElementDescPerVertex[1].SemanticIndex = 0;
	inputElementDescPerVertex[1].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	inputElementDescPerVertex[1].InputSlot = 1;
	inputElementDescPerVertex[1].AlignedByteOffset = 0;
	inputElementDescPerVertex[1].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	inputElementDescPerVertex[1].InstanceDataStepRate = 0;

	hr = gpID3D11Device->CreateInputLayout(inputElementDescPerVertex,
		_ARRAYSIZE(inputElementDescPerVertex),
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateInputLayout() Per Vertex Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateInputLayout() Per Vertex Succeeded.\n");
		fclose(gpFile);
	}
	//Default Setting
	gpID3D11DeviceContext->IASetInputLayout(gpID3D11InputLayoutPerVertex);
	...
	ID3DBlob* pID3DBlob_ErrorPerPixel = NULL;

	hr = D3DCompile(vertexShaderSourceCodePerPixel,
		lstrlenA(vertexShaderSourceCodePerPixel) + 1,
		"VS",
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
		...
		&pID3DBlob_ErrorPerPixel);

	if (FAILED(hr))
	{
		if (pID3DBlob_ErrorPerPixel != NULL)
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "D3DCompile() Failed For Vertex Shader Per Pixel : %s.\n", (char*)pID3DBlob_ErrorPerPixel->GetBufferPointer());
			fclose(gpFile);
			pID3DBlob_ErrorPerPixel->Release();
			pID3DBlob_ErrorPerPixel = NULL;
			return(hr);
		}
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "D3DCompile() Succeeded For Vertex Shader Per Pixel.\n ");
		fclose(gpFile);
	}
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateVertexShader() Per Pixel Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateVertexShader() Per Pixel Succeeded. \n ");
		fclose(gpFile);
	}
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreatePixelShader() Per Pixel Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreatePixelShader() Per Pixel Succeeded.\n ");
		fclose(gpFile);
	}

	//create and set input layout
	D3D11_INPUT_ELEMENT_DESC inputElementDescPerPixel[2];
	inputElementDescPerPixel[0].SemanticName = "POSITION";
	inputElementDescPerPixel[0].SemanticIndex = 0;
	inputElementDescPerPixel[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	inputElementDescPerPixel[0].InputSlot = 0;
	inputElementDescPerPixel[0].AlignedByteOffset = 0;
	inputElementDescPerPixel[0].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	inputElementDescPerPixel[0].InstanceDataStepRate = 0;

	inputElementDescPerPixel[1].SemanticName = "NORMAL";
	inputElementDescPerPixel[1].SemanticIndex = 0;
	inputElementDescPerPixel[1].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	inputElementDescPerPixel[1].InputSlot = 1;
	inputElementDescPerPixel[1].AlignedByteOffset = 0;
	inputElementDescPerPixel[1].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	inputElementDescPerPixel[1].InstanceDataStepRate = 0;

	hr = gpID3D11Device->CreateInputLayout(inputElementDescPerPixel,
		_ARRAYSIZE(inputElementDescPerPixel),
		pID3DBlob_VertexShaderCodePerPixel->GetBufferPointer(),
		pID3DBlob_VertexShaderCodePerPixel->GetBufferSize(),
		&gpID3D11InputLayoutPerPixel);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateInputLayout() Per Pixel Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateInputLayout() Per Pixel Succeeded.\n");
		fclose(gpFile);
	}
...
void uninitialize(void)
{
	...
	if (gpID3D11InputLayoutPerPixel)
	{
		gpID3D11InputLayoutPerPixel->Release();
		gpID3D11InputLayoutPerPixel = NULL;
	}
	...
	if (gpID3D11PixelShaderPerPixel)
	{
		gpID3D11PixelShaderPerPixel->Release();
		gpID3D11PixelShaderPerPixel = NULL;
	}
	if (gpID3D11VertexShaderPerPixel)
	{
		gpID3D11VertexShaderPerPixel->Release();
		gpID3D11VertexShaderPerPixel = NULL;
	}
