...
HRESULT initialize(void)
{
	...
	const char* vertexShaderSourceCode =
		...
		"	float3 lightDirectionZero	:	NORMAL1;" \
		"	float3 lightDirectionOne	:	NORMAL2;" \
		"	float3 lightDirectionTwo	:	NORMAL3;" \
		...
		"       output.lightDirectionZero = (float3)(lightPositionZero.xyz - eye_coordinates.xyz);" \
		"       output.lightDirectionOne = (float3)(lightPositionOne.xyz - eye_coordinates.xyz);" \
		"       output.lightDirectionTwo = (float3)(lightPositionTwo.xyz - eye_coordinates.xyz);" \
	...
	const char* pixelShaderSourceCode =
		...
		"		\n" \
		"		float3 normalizedLightDirectionZero = normalize(output.lightDirectionZero);" \
		"		float tn_dot_ld_Zero = max(dot(normalizedLightDirectionZero, normalizedTransformedNormal), 0.0);" \
		"		float4 ambientZero = laZero * ka;" \
		"		float4 diffuseZero = ldZero * kd * tn_dot_ld_Zero;" \
		"		float3 reflectionVectorZero = reflect(-normalizedLightDirectionZero, normalizedTransformedNormal);" \
		"		float4 specularZero = lsZero * ks * pow(max(dot(reflectionVectorZero, normalizedViewVector), 0.0), materialShininess);" \
		"		\n" \
		"		float3 normalizedLightDirectionOne = normalize(output.lightDirectionOne);" \
		"		float tn_dot_ld_One = max(dot(normalizedLightDirectionOne, normalizedTransformedNormal), 0.0);" \
		"		float4 ambientOne = laOne * ka;" \
		"		float4 diffuseOne = ldOne * kd * tn_dot_ld_One;" \
		"		float3 reflectionVectorOne = reflect(-normalizedLightDirectionOne, normalizedTransformedNormal);" \
		"		float4 specularOne = lsOne * ks * pow(max(dot(reflectionVectorOne, normalizedViewVector), 0.0), materialShininess);" \
		"		\n" \
		"		float3 normalizedLightDirectionTwo = normalize(output.lightDirectionTwo);" \
		"		float tn_dot_ld_Two = max(dot(normalizedLightDirectionTwo, normalizedTransformedNormal), 0.0);" \
		"		float4 ambientTwo = laTwo * ka;" \
		"		float4 diffuseTwo = ldTwo * kd * tn_dot_ld_Two;" \
		"		float3 reflectionVectorTwo = reflect(-normalizedLightDirectionTwo, normalizedTransformedNormal);" \
		"		float4 specularTwo = lsTwo * ks * pow(max(dot(reflectionVectorTwo, normalizedViewVector), 0.0), materialShininess);" \
		"		\n" \
...
void display(void)
{
	...
	if (gbLighting == true)
	{
		...
		lightZeroPosition[1] = sinf(angle) * 100.0f;//only sin()for double type... sinef() for float type....
		lightZeroPosition[2] = cosf(angle) * 100.0f + 3.0f;

		lightOnePosition[0] = sinf(angle) * 100.0f;
		lightOnePosition[1] = 0.0f;
		lightOnePosition[2] = cosf(angle) * 100.0f + 3.0f;

		lightTwoPosition[0] = cosf(angle) * 100.0f;
		lightTwoPosition[1] = sinf(angle) * 100.0f;
		lightTwoPosition[2] = 3.0f;

		constantBuffer.isLightingEnabled = 1;
		constantBuffer.laZero = XMVectorSet(lightZeroAmbient[0], lightZeroAmbient[1], lightZeroAmbient[2], lightZeroAmbient[3]);
		constantBuffer.ldZero = XMVectorSet(lightZeroDiffuse[0], lightZeroDiffuse[1], lightZeroDiffuse[2], lightZeroDiffuse[3]);
		constantBuffer.lsZero = XMVectorSet(lightZeroSpecular[0], lightZeroSpecular[1], lightZeroSpecular[2], lightZeroSpecular[3]);
		constantBuffer.laOne = XMVectorSet(lightOneAmbient[0], lightOneAmbient[1], lightOneAmbient[2], lightOneAmbient[3]);
		constantBuffer.ldOne = XMVectorSet(lightOneDiffuse[0], lightOneDiffuse[1], lightOneDiffuse[2], lightOneDiffuse[3]);
		constantBuffer.lsOne = XMVectorSet(lightOneSpecular[0], lightOneSpecular[1], lightOneSpecular[2], lightOneSpecular[3]);
		constantBuffer.laTwo = XMVectorSet(lightTwoAmbient[0], lightTwoAmbient[1], lightTwoAmbient[2], lightTwoAmbient[3]);
		constantBuffer.ldTwo = XMVectorSet(lightTwoDiffuse[0], lightTwoDiffuse[1], lightTwoDiffuse[2], lightTwoDiffuse[3]);
		constantBuffer.lsTwo = XMVectorSet(lightTwoSpecular[0], lightTwoSpecular[1], lightTwoSpecular[2], lightTwoSpecular[3]);
		constantBuffer.lightZeroPosition = XMVectorSet(lightZeroPosition[0], lightZeroPosition[1], lightZeroPosition[2], lightZeroPosition[3]);
		constantBuffer.lightOnePosition = XMVectorSet(lightOnePosition[0], lightOnePosition[1], lightOnePosition[2], lightOnePosition[3]);
		constantBuffer.lightTwoPosition = XMVectorSet(lightTwoPosition[0], lightTwoPosition[1], lightTwoPosition[2], lightTwoPosition[3]);
